...
def criar_relacionamento_contador_empresa(cnpj_contador, cnpj_empresa, tipo_relacao='Associado'):
    try:
        cursor_relacionamentos.execute("""
            INSERT INTO contador_empresa
              (cnpj_contador, cnpj_empresa, tipo_relacao)
            VALUES (?, ?, ?)
        """, (cnpj_contador, cnpj_empresa, tipo_relacao)
        )
        ...
        print(f"Relacionamento entre contador {cnpj_contador} e empresa {cnpj_empresa} já existe.")
        return False

def buscar_empresas_por_contador(cnpj_contador):
    cursor_relacionamentos.execute("""
        SELECT e.* FROM empresas e
        JOIN contador_empresa ce ON e.cnpj = ce.cnpj_empresa
        WHERE ce.cnpj_contador = ? AND ce.ativo = 1
    """, (cnpj_contador,))
    return cursor_relacionamentos.fetchall()
...
def buscar_contadores_por_empresa(cnpj_empresa):
    cursor_relacionamentos.execute("""
        SELECT c.* FROM contadores_novo c
        JOIN contador_empresa ce ON c.cnpj = ce.cnpj_contador
        WHERE ce.cnpj_empresa = ? AND ce.ativo = 1
    """, (cnpj_empresa,))
    return cursor_relacionamentos.fetchall()

def desativar_relacionamento_contador_empresa(cnpj_contador, cnpj_empresa):
    cursor_relacionamentos.execute("""
        UPDATE contador_empresa SET ativo = 0
        WHERE cnpj_contador = ? AND cnpj_empresa = ?
    """, (cnpj_contador, cnpj_empresa)
    )
    conn_relacionamentos.commit()

def ativar_relacionamento_contador_empresa(cnpj_contador, cnpj_empresa):
    cursor_relacionamentos.execute("""
        UPDATE contador_empresa SET ativo = 1
        WHERE cnpj_contador = ? AND cnpj_empresa = ?
    """, (cnpj_contador, cnpj_empresa)
    )
    conn_relacionamentos.commit()

def criar_relacionamento_empresa_repis(cnpj_empresa, cnpj_repis, ano_repis, cnpj_contador_responsavel, status='Ativo'):
    try:
        cursor_relacionamentos.execute("""
            INSERT INTO empresa_repis
              (cnpj_empresa, cnpj_repis, ano_repis, cnpj_contador_responsavel, status)
            VALUES (?, ?, ?, ?, ?)
        """, (cnpj_empresa, cnpj_repis, ano_repis, cnpj_contador_responsavel, status)
        )
        ...
        print(f"Relacionamento entre empresa {cnpj_empresa} e REPIS {cnpj_repis} já existe.")
        return False

def buscar_repis_por_empresa(cnpj_empresa):
    cursor_relacionamentos.execute("""
        SELECT r.* FROM repis r
        JOIN empresa_repis er ON r.cnpj = er.cnpj_repis
        WHERE er.cnpj_empresa = ?
    """, (cnpj_empresa,))
    return cursor_relacionamentos.fetchall()

def buscar_empresas_por_repis(cnpj_repis):
    cursor_relacionamentos.execute("""
        SELECT e.* FROM empresas e
        JOIN empresa_repis er ON e.cnpj = er.cnpj_empresa
        WHERE er.cnpj_repis = ?
    """, (cnpj_repis,))
    return cursor_relacionamentos.fetchall()

def atualizar_relacionamento_empresa_repis(cnpj_empresa, cnpj_repis, **kwargs):
    campos = []
    valores = []
    ...
    valores.append(cnpj_empresa)
    valores.append(cnpj_repis)
    sql = f"UPDATE empresa_repis SET {", ".join(campos)} WHERE cnpj_empresa = ? AND cnpj_repis = ?"
    cursor_relacionamentos.execute(sql, valores)
    conn_relacionamentos.commit()

def excluir_relacionamento_empresa_repis(cnpj_empresa, cnpj_repis):
    cursor_relacionamentos.execute("DELETE FROM empresa_repis WHERE cnpj_empresa = ? AND cnpj_repis = ?", (cnpj_empresa, cnpj_repis))
    conn_relacionamentos.commit()

def buscar_contador_por_cnpj_ou_nome_com_empresas(termo):
    """
    Busca contador por CNPJ ou Nome e retorna suas empresas associadas.
    """
    contador = buscar_contador_por_cnpj_ou_nome(termo)
    if contador:
        cnpj_contador = contador[0]
        empresas_associadas = buscar_empresas_por_contador(cnpj_contador)
        return contador, empresas_associadas
    return None, None
...
def buscar_contador_por_cnpj_ou_nome_com_empresas(termo):
    ...
    # Buscar o contador
    cursor.execute("SELECT * FROM contador WHERE cnpj = ? OR nome = ?", (termo, termo))
    contador_info = cursor.fetchone()
    
    empresas_associadas = []
    if contador_info:
        contador_id = contador_info[0]
        # Buscar as empresas associadas a este contador
        cursor.execute("SELECT empresa.cnpj, empresa.razao_social FROM contador_empresa JOIN empresa ON contador_empresa.empresa_id = empresa.id WHERE contador_empresa.contador_id = ?", (contador_id,))
        empresas_associadas = cursor.fetchall()
        
    conn.close()
    return contador_info, empresas_associadas
